...
//jenkins agent label
//test/deploy.Jenkinsfile：test环境的独立部署job，跟test/astrox.Jenkinsfile（构建job）完全独立、互不影响
//结构照抄dev/uat的纯部署模式：只做helm部署，不checkout业务代码/build/push，镜像由test构建job统一产出
//跟dev/uat不同的一点：本job和test构建job读的是同一份test/setting.groovy，namespaces直接在本文件内解析，不需要像dev/uat那样跨文件读test/setting.groovy
//先用内置静态agent clone一次配置仓库，只为了读出该用哪个K8s cloud/哪个部署agent镜像
node {
    stage('Resolve deploy agent') {
        ...
        //部署agent镜像跟构建agent镜像分开：部署只需要helm/kubectl/awscli，不需要JDK/Maven/buildah
        env.agent_image = (code_info.private.deploy_agent_image).toString()
    }
}
        ...
        container('deploy') {
            try {
                ...
                stage('Check info') {
                    ...
                    //读取配置文件（跟test构建job共用同一份test/setting.groovy，这里只取部署相关字段）
                    def file = readFile("astrox-helm-chart/test/setting.groovy")
                    def jsonSlurper = new JsonSlurper()
                    def code_info = jsonSlurper.parseText(file)
                    ...
                    //公共参数
                    //env_tier固定等于本环境目录名（test），用于定位chart源码目录/node taint；namespaces是真正的k8s namespace，可被服务级配置覆盖
                    ...
                    //跟test构建job同一份配置文件，namespaces直接本环境内解析，就是test构建时实际push的路径
                ...
                //加载pipeline流程（只负责接收SPECIFY_TAG/IMAGE_TAG参数，不checkout+build+push）
                ...
                stage('update helm chart') {
                    sh '''
                        timeout 380 kubectl --kubeconfig ${KUBECONFIG} rollout status ${kind_name} -n ${namespaces} ${app_name} || status=false
                ...
                sh "curl -s -X POST -H 'Content-Type: application/json' -d '{\"msg_type\":\"text\",\"content\":{\"text\":\"[${env.JOB_BASE_NAME}] test部署成功 tag=${env.image_tag}\"}}' ${env.lark_webhook_url} || true"
            } catch (e) {
                sh "curl -s -X POST -H 'Content-Type: application/json' -d '{\"msg_type\":\"text\",\"content\":{\"text\":\"[${env.JOB_BASE_NAME}] test部署失败\"}}' ${env.lark_webhook_url} || true"
                throw e
            }
        }
    }
