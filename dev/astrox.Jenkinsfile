    ...
    node(POD_LABEL) {
        container('deploy') {
            try {
                ...
                sh "curl -s -X POST -H 'Content-Type: application/json' -d '{\"msg_type\":\"text\",\"content\":{\"text\":\"[${env.JOB_BASE_NAME}] dev部署成功 tag=${env.image_tag}\"}}' ${env.lark_webhook_url} || true"
            } catch (e) {
                sh "curl -s -X POST -H 'Content-Type: application/json' -d '{\"msg_type\":\"text\",\"content\":{\"text\":\"[${env.JOB_BASE_NAME}] dev部署失败\"}}' ${env.lark_webhook_url} || true"
                throw e
            }
        }
    }
