...
properties([
    parameters([
        string(name: 'BRANCH_TAG', defaultValue: 'main', description: 'dev是第一梯队构建环境，默认值main=用这个分支checkout+build+push新镜像再部署；留空=跳过构建，直接部署test已构建好的镜像(用下面IMAGE_TAG，或自动取ECR最新tag)。分支名手动输入，如果打错/该分支不存在，会在下面"Validate branch"阶段直接报错终止，不会跑到一半才失败'),
