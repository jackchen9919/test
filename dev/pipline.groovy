pipeline {
    agent any
    //IMAGE_TAG参数已挪到外层dev/astrox.Jenkinsfile的properties()统一声明（这里原来的parameters{}块在load()子pipeline里不会注册成真正job参数）
    stages {
                ...
                script {
                    if (params.IMAGE_TAG?.trim()) {
                        ...
                        if (!env.image_tag || env.image_tag == 'None') {
                            error "自动获取ECR最新tag失败，请确认test job至少成功构建过一次，或手动指定IMAGE_TAG"
                        }
