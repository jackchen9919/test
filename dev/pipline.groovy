pipeline {
    agent any
    //SPECIFY_TAG/IMAGE_TAG参数已挪到外层dev/astrox.Jenkinsfile的properties()统一声明（这里原来的parameters{}块在load()子pipeline里不会注册成真正job参数）
    stages {
            ...
            steps {
                script {
                    if (params.SPECIFY_TAG) {
                        ...
                        env.image_tag = sh(
                            script: "aws ecr describe-images --repository-name ${test_namespaces}/${app_name} --region ${aws_region} --query 'sort_by(imageDetails,&imagePushedAt)[-1].imageTags[0]' --output text",
                            returnStdout: true
                        ).trim()
                        if (!env.image_tag || env.image_tag == 'None') {
                            error "自动获取ECR最新tag失败，请确认test job至少成功构建过一次，或勾选SPECIFY_TAG手动指定IMAGE_TAG"
                        }
