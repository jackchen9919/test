pipeline {
    ...
    //BRANCH_TAG参数已挪到外层uat/astrox.Jenkinsfile的properties()统一声明（这里原来的parameters{}块在load()子pipeline里不会注册成真正job参数）
        ...
        stage('Build/Push') {
            steps {
                ...
                script {
                    ...
                    // build and pushing —— uat自行构建，直接推到跟test共用的ECR路径（test_namespaces），保持跟部署路径消费的镜像地址一致
                    ...
                    sh """
                        ...
                        # 额外打latest标签并push：sit/dev"跳过构建直接部署"路径靠这个latest标签取真正最新的镜像，
