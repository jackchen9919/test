...
`test/` 目录下只有**一个job**（`test/astrox.Jenkinsfile`），构建和部署合并在一起——是否构建由 `BRANCH_TAG` 参数选的是不是`SKIP_BUILD`控制（选真实分支名=先checkout业务代码+build+push再部署，走`test/pipline.groovy`；选`SKIP_BUILD`=跳过构建直接部署，走`test/deploy_pipline.groovy`），跟`prod`一样是"build+deploy同job"模式，build不是单独的job，只是job里"选哪个分支/选SKIP_BUILD"这一个开关。`BRANCH_TAG`是Active Choices（uno-choice插件）动态下拉框，脚本实时跑`git ls-remote`列出业务仓库真实存在的分支供选，`SKIP_BUILD`是列表里固定排在最前的一个字面选项（不是留空/不是下拉框拉取失败的意思）；即便如此，pipeline里仍保留一道`git ls-remote`兜底校验（防止直接调API绕过下拉框传入无效分支名），不存在直接报错终止，不会等到build跑到一半才失败。

`dev`、`uat` 两个环境也是同样的`BRANCH_TAG`下拉开关模式：默认选中`SKIP_BUILD`=纯部署（不checkout业务代码/build/push，用`IMAGE_TAG`——留空自动去ECR查该服务最新push的tag，填了则用这个值部署，用于回滚/部署指定历史版本）；选真实分支名=脱离test单独构建部署（dev默认选中项是`main`排第一，uat默认选中项是`SKIP_BUILD`排第一）。
...
### test（构建 + 部署，同一个job，`BRANCH_TAG`下拉选的是不是`SKIP_BUILD`控制是否构建）
```
1）修改 test/setting.groovy，新增子字典（jenkins项目名做key，如没有将退出）——构建字段和部署字段写在同一个字典里：
    "xxx-service": {
        # 构建相关字段（BRANCH_TAG下拉选真实分支时用）
...
2）Build with Parameters跑job：`BRANCH_TAG`下拉选一个真实分支（如`main`，列表是实时`git ls-remote`拉取的）=先构建新镜像（走`test/pipline.groovy`，构建前还会再用`git ls-remote`兜底校验一次该分支真实存在）再部署；选`SKIP_BUILD`=跳过构建直接部署（`IMAGE_TAG`留空直接跑即可，自动去ECR取该服务最新一次push的tag）
```
构建工具是 buildah（不是 docker），构建前会先 `aws ecr get-login-password | buildah login` 显式登录ECR，ECR仓库不存在会自动 `aws ecr create-repository` 创建。`BRANCH_TAG`选真实分支时用`private.agent_image`（JDK17/Maven/buildah规格），选`SKIP_BUILD`时用`private.deploy_agent_image`（helm/kubectl/awscli规格），同一个job按参数二选一。
...
```
prod的agent镜像（`private.agent_image`）需要同时具备构建工具链（JDK17/Maven/buildah）和部署工具链（helm/kubectl/awscli），因为prod始终构建，build+deploy在同一个job里跑（跟test不同的是prod没有`BRANCH_TAG`下拉框/`SKIP_BUILD`这个跳过构建的开关，永远构建）。
